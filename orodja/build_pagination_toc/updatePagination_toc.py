...
def updatePagination(thisUnit, prevUnit, nextUnit, pages, lastInPrevUnit, pastPages):
    cwd = os.getcwd()
    if prevUnit == '': prevUnit = None
    if nextUnit == '': nextUnit = None
    unit = {'id': thisUnit, 'pages': pages, 'lastInPrevUnit': lastInPrevUnit, 'previous': prevUnit, 'next': nextUnit}

    for page in range(0, unit['pages']):
        if page == 0:
            filename =  cwd + "/" + thisUnit + "/index.html"
        else:
            filename = cwd + "/" + thisUnit + "/index" + str(page) + ".html"
        print(filename)
...
def main():
    ...
    print("===================")
    tocFilename = input("Enter filename that contains TOC (leave empty for 'TOC.txt'): ")
    if tocFilename == '':
        tocFilename = "TOC.txt"

    # Process TOC file
    file = open(tocFilename, "r", encoding="utf8")
    units = file.read().splitlines()

    # Initialize variables
    numberOfUnits = len(units)-1
    lastInPrevUnit = 0
    pastPages = 0
    
    for index in range(0, len(units)):
        folder = os.getcwd() + "/" + units[index] + "/index*.html"
        filelist = glob.glob(folder)
        numberOfFiles = len(filelist)

        # For each unit extract information about current, previous and next unit
        thisUnit = units[index]
        prevUnit = None
        if index > 0:
            prevUnit = units[index-1]
        nextUnit = None
        if index < numberOfUnits:
            nextUnit = units[index+1]

        print("Updating pagination in unit", thisUnit, "...")
        if DEBUG:
            print("This unit:", thisUnit)
            print("Prev unit:", prevUnit)
            print("Next unit:", nextUnit)
            print("Pages in this unit:", numberOfFiles)
            print("Pages in previous unit:", lastInPrevUnit)
            print("Past total pages:", pastPages)
            print("----------")

        # Actually update pagination in all files (index*.html) of the current unit
        updatePagination(thisUnit, prevUnit, nextUnit, numberOfFiles, lastInPrevUnit, pastPages)

        # Update information about number of pages in current unit, which will become
        # number of pages in previous unit in next iteration of the loop, as well as
        # the total number of pages in already processed units
        lastInPrevUnit = numberOfFiles
        pastPages += numberOfFiles
